...
function setupButtonInteractions() {
  ...
  glyphButton.addEventListener('mousedown', handlePressStart)
  glyphButton.addEventListener('touchstart', handlePressStart)
  glyphButton.addEventListener('mouseup', handlePressEnd)
  glyphButton.addEventListener('touchend', handlePressEnd)
  glyphButton.addEventListener('touchcancel', handlePressCancel)
  glyphButton.addEventListener('mouseleave', handlePressCancel)
}
...
function handleActionUp() {
  ...
  // Do NOT remove .pressed here; only remove in handlePressEnd
}
...
// Stop running code
function stopCode() {
  const hadRunningCode =
    isCodeRunning ||
    runningTimeouts.size > 0 ||
    runningIntervals.size > 0 ||
    runningAnimations.size > 0
...
function createInteractiveToy() {
  ...
  function updateToyDisplay() {
    ...
    const frame = new GlyphMatrixFrame.Builder().addTop(object).build()
...
window.GlyphEffects = {
  fadeIn: function (manager, object, duration = 1000) {
    ...
    function animate() {
      ...
      const frame = new GlyphMatrixFrame.Builder().addTop(fadedObject).build()
